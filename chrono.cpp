...
int i=0;
      ...
      TString fname;
      TIter next(files);
      while ((file=(TSystemFile*)next()) and i<25) { if(i<10){i++; continue;}
...
Long64_t nevtot = chain.GetEntries();
  ...
  //gROOT->SetBatch(kFALSE); 
	...
	chain.SetBranchAddress("Event", &event);
        ...
        std::vector<TH1F*> intensity;
	intensity.push_back(new TH1F("int0", "intensity mod0", 1000,-5,5));
        intensity.push_back(new TH1F("int1", "intensity mod1", 1000,-5,5));
        intensity.push_back(new TH1F("int2", "intensity mod2", 1000,-5,5));
        intensity.push_back(new TH1F("int3", "intensity mod3", 1000,-5,5));
        intensity.push_back(new TH1F("int4", "intensity mod4", 1000,-5,5));
        intensity.push_back(new TH1F("int5", "intensity mod5", 1000,-5,5));
...
double stubU1,stubU2,stubU3=0;
double stubV1,stubV2,stubV3=0;
double stub4=0;
...
for(Long64_t i = 0; i < chain.GetEntries(); i++) {
	...
	vector<MUonERealDataStub> stub= event->stubs();
        vector<int> nstubs_per_mod_BX(6,0);
        vector<vector<MUonERealDataStub>> stubs_mod0(40);

// U and V from strip number to cm in local reference frame
                auto absU_=[&](int mod,int stub,vector<vector<MUonERealDataStub>> stubs_mod){ double posStripU = stubs_mod.at(mod).at(stub).localX()/2. -1;
                                      return (- (posStripU - 1016/2.)*0.009 -0.009/2.);};
                auto absV_=[&](int mod,int stub,vector<vector<MUonERealDataStub>> stubs_mod){ double posStripV = stubs_mod.at(mod).at(stub).localX()/2. -1;
                                      return ( + (posStripV - 1016/2.)*0.009 +0.009/2.);};
// X and Y from strip number to cm in local reference frame
                auto absX_=[&](int mod,int stub,vector<vector<MUonERealDataStub>> stubs_mod){ double posStripU = stubs_mod.at(mod).at(stub).localX()/2. -1;
                                      return (cos(0.233)*(- (posStripU - 1016/2.)*0.009 -0.009/2.));};
                auto absY_=[&](int mod,int stub,vector<vector<MUonERealDataStub>> stubs_mod){ double posStripV = stubs_mod.at(mod).at(stub).localX()/2. -1;
...
// m and q parameters for XZ and YZ track
                auto l_mXZ = [&](int stub_mod0, int stub_mod4,vector<vector<MUonERealDataStub>> stubs_mod){return ((absX_(4,stub_mod4,stubs_mod)+Xoffsets[4])-(absX_(0,stub_mod0,stubs_mod)+Xoffsets[0]))/(absZ04_(4,absX_(4,stub_mod4,stubs_mod),Xoffsets[4])-absZ04_(0,absX_(0,stub_mod0,stubs_mod),Xoffsets[0]));};
                auto l_mYZ = [&](int stub_mod1, int stub_mod5,vector<vector<MUonERealDataStub>> stubs_mod){return ((absY_(5,stub_mod5,stubs_mod)+Yoffsets[5])-(absY_(1,stub_mod1,stubs_mod)+Yoffsets[1]))/(absZ15_(5,absY_(5,stub_mod5,stubs_mod),Yoffsets[5])-absZ15_(1,absY_(1,stub_mod1,stubs_mod),Yoffsets[1]));};
                auto l_qXZ = [&](int stub_mod0, int stub_mod4,vector<vector<MUonERealDataStub>> stubs_mod){return (absZ04_(4,absX_(4,stub_mod4,stubs_mod),Xoffsets[4])*(absX_(0,stub_mod0,stubs_mod)+Xoffsets[0]) - absZ04_(0,absX_(0,stub_mod0,stubs_mod),Xoffsets[0])*(absX_(4,stub_mod4,stubs_mod)+Xoffsets[4]))/(absZ04_(4,absX_(4,stub_mod4,stubs_mod),Xoffsets[4])-absZ04_(0,absX_(0,stub_mod0,stubs_mod),Xoffsets[0]));};
                auto l_qYZ = [&](int stub_mod1, int stub_mod5,vector<vector<MUonERealDataStub>> stubs_mod){return (absZ15_(5,absY_(5,stub_mod5,stubs_mod),Yoffsets[5])*(absY_(1,stub_mod1,stubs_mod)+Yoffsets[1]) - absZ15_(1,absY_(1,stub_mod1,stubs_mod),Yoffsets[1])*(absY_(5,stub_mod5,stubs_mod)+Yoffsets[5]))/(absZ15_(5,absY_(5,stub_mod5,stubs_mod),Yoffsets[5])-absZ15_(1,absY_(1,stub_mod1,stubs_mod),Yoffsets[1]));};
...
// X or Y position on the track at a given Z
        ...
        for(int i = 0; i<stub.size(); i++)
        {
         ...
         stubs_mod0.at(link).push_back(stub.at(i));
        }

/*        for(int s = 0; s<stubs_mod0.at(0).size(); s++){intensity.at(0)->Fill(absU_(0,s,stubs_mod0));}
        for(int s = 0; s<stubs_mod0.at(1).size(); s++){intensity.at(1)->Fill(absV_(1,s,stubs_mod0));}
        for(int s = 0; s<stubs_mod0.at(2).size(); s++){intensity.at(2)->Fill(absU_(2,s,stubs_mod0));}
        for(int s = 0; s<stubs_mod0.at(3).size(); s++){intensity.at(3)->Fill(absV_(3,s,stubs_mod0));}
        for(int s = 0; s<stubs_mod0.at(4).size(); s++){intensity.at(4)->Fill(absU_(4,s,stubs_mod0));}
        for(int s = 0; s<stubs_mod0.at(5).size(); s++){intensity.at(5)->Fill(absV_(5,s,stubs_mod0));}
*/
...
/*cout << absZ04_(4,absX_(4,0,stubs_mod0),Xoffsets[4]) << " v4s " << stubs_mod0.at(4).at(0).absZ() << endl;
cout << absZ15_(1,absX_(1,0,stubs_mod0),Xoffsets[1]) << " v1s " << stubs_mod0.at(1).at(0).absZ() << endl;
cout << absY_(1,0,stubs_mod0)<< " vy1s " << stubs_mod0.at(1).at(0).absY() << endl;
*/
...
chain.GetEntry(i-1);
cout <<  stub2.size() << endl;
        vector<MUonERealDataStub> stub1= event->stubs();
        vector<int> nstubs_per_mod_BX1(6,0);
        vector<vector<MUonERealDataStub>> stubs_mod1(40);

        for(int i = 0; i<stub1.size(); i++)
        {
         int link= stub1.at(i).link();
         nstubs_per_mod_BX1.at(link)+=1;
         stubs_mod1.at(link).push_back(stub1.at(i));
        }
double res_U1;
...
if(two==1){stubU1++;h_bx0->Fill(event->bx()-Bx); }
two=0;
chain.GetEntry(i);

chain.GetEntry(i+1);

        vector<MUonERealDataStub> stub2= event->stubs();
        vector<int> nstubs_per_mod_BX2(6,0);
        vector<vector<MUonERealDataStub>> stubs_mod2(40);

        for(int i = 0; i<stub2.size(); i++)
        {
         int link= stub2.at(i).link();
         nstubs_per_mod_BX2.at(link)+=1;
         stubs_mod2.at(link).push_back(stub2.at(i));
        }
 double res_U2;
...
if(two==1){stubU2++; h_bx1->Fill(event->bx()-Bx);        residual_Y->Fill(res_U2);}
two=0;
chain.GetEntry(i);
...
chain.GetEntry(i-1);
        ...
        vector<MUonERealDataStub> stub1_v= event->stubs();
        vector<int> nstubs_per_mod_BX1_v(6,0);
        vector<vector<MUonERealDataStub>> stubs_mod1_v(40);

        for(int i = 0; i<stub1_v.size(); i++)
        {
         int link= stub1_v.at(i).link();
         nstubs_per_mod_BX1_v.at(link)+=1;
         stubs_mod1_v.at(link).push_back(stub1_v.at(i));
        }
...
if(two==1){stubV1++; h_bx0_v->Fill(event->bx()-Bx_v);}
two=0;

chain.GetEntry(i);
chain.GetEntry(i+1);

        vector<MUonERealDataStub> stub2_v= event->stubs();
        vector<int> nstubs_per_mod_BX2_v(6,0);
        vector<vector<MUonERealDataStub>> stubs_mod2_v(40);

        for(int i = 0; i<stub2_v.size(); i++)
        {
         int link= stub2_v.at(i).link();
         nstubs_per_mod_BX2_v.at(link)+=1;
         stubs_mod2_v.at(link).push_back(stub2_v.at(i));
        }
...
if(two==1){stubV2++; h_bx1_v->Fill(event->bx()-Bx_v);}
two=0;
...
for(int i=0; i<6; i++){intensity.at(i)->Sumw2();}

TCanvas n3("i","i", 700,700);
n3.Divide(2,3);
n3.cd(1);
intensity.at(0)->Draw();
n3.cd(2);
